...
// UNO R3 Version
// Tek Logo Demo  - R2R Version
//
// Bug Fixed by DoKA, 2023/12/31
// PORTC = y & 3;  ->  PORTC = (x << 2) | (y & 3);
// 890 lines
// #if 0  ->  #if 1
...
//PROGMEM  prog_uchar starburst[] = {
PROGMEM  const unsigned char starburst[] = {

//starburst (radial lines) for testing DDA's
...
void draw(unsigned char *image, int imagesize) {
...
void setup() {
  ...
  //pinMode (PORTC0, OUTPUT);
  //pinMode (PORTC1, OUTPUT);
  // but this does
  DDRC |= 3;
...
{
    int i;
    // change the following "0" to a "1" to generate ramps for DAC testing
#if 0
    // ramps for DAC testing:
    for (i = 0; i < 256; i++) {
        plot(i, i);
    }
#else
    // normal image drawing; draw each 500 times
    for (i = 0; i < 500; i++) {
        draw(wizardidd, sizeof(wizardidd));
    }
    for (i = 0; i < 500; i++) {
        draw(logo, sizeof(logo));
    } 
    // change the following "0" to "1" to add the starburst 
#if 0
for (i = 0; i < 3000; i++) {
        draw(starburst, sizeof(starburst));
    }
#endif
